...
limiter = Limiter(
    ...
    key_prefix="resume_ocr",

    app=app,

    default_limits=["100 per day", "10 per hour"], # 기본 제한
...
"""
        ...
        if not all(k in coords for k in required_keys):
            logger.warning("프로필 사진 추출 실패: 좌표 키 누락")
            return None


        try:
            x_min = int(float(coords.get('x_min', 0)) * width / 1000)
            y_min = int(float(coords.get('y_min', 0)) * height / 1000)
            x_max = int(float(coords.get('x_max', 1000)) * width / 1000)
            y_max = int(float(coords.get('y_max', 1000)) * height / 1000)
        except ValueError:
           logger.warning("프로필 사진 추출 실패: 좌표 값 변환 오류")
           return None
...
def _process_resume_extraction():
    ...
    try:
        ...
        try:
            ...
            processed_image = None  # 주 데이터 추출을 위한 최종 이미지 (PDF의 경우 합쳐진 이미지)

            image_for_photo_detection = None # 프로필 사진 좌표 감지를 위한 이미지 (PDF의 경우 1페이지)
            ...
            if file_extension == '.pdf':
                ...
                # 1. 프로필 사진 감지용: 1페이지만 추출 (DPI 100 권장)

                profile_img_page_list = convert_from_bytes(image_bytes, first_page=1, last_page=1, dpi=100)

                if profile_img_page_list:

                    image_for_photo_detection = profile_img_page_list[0]

                   

                # 2. 주 데이터 추출용: 1~3페이지 추출 (DPI 72 권장)

                images_for_extraction = convert_from_bytes(image_bytes, first_page=1, last_page=3, dpi=72)



                if images_for_extraction:

                    # 모든 이미지를 수직으로 이어 붙이기 (Concatenation)

                    widths, heights = zip(*(i.size for i in images_for_extraction))
                ...
                image_for_photo_detection = processed_image # 이미지 파일은 하나만 있으므로 동일하게 사용



            # 3. 이미지 크기 최적화 및 RGBA to RGB 변환 (processed_image)
            ...
            # 4. 이미지 크기 최적화 및 RGBA to RGB 변환 (image_for_photo_detection)

            if image_for_photo_detection:

                if image_for_photo_detection.mode == 'RGBA':

                    rgb_image_photo = Image.new('RGB', image_for_photo_detection.size, (255, 255, 255))

                    rgb_image_photo.paste(image_for_photo_detection, mask=image_for_photo_detection.split()[-1])

                    image_for_photo_detection = rgb_image_photo

                   

            logger.info(f"파일 처리 완료: 추출 이미지 크기 {processed_image.size}, 사진 감지 이미지 존재: {image_for_photo_detection is not None}")
        ...
        # 5. 프로필 사진 추출 시도 (올바른 단일 페이지 이미지 사용)

        profile_photo_base64 = None

        if image_for_photo_detection:

            profile_photo_base64 = detect_and_extract_profile_photo(image_for_photo_detection)
...
@limiter.limit("5 per minute; 50 per day", override_defaults=False)
